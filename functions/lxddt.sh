...
function lxddt {
  ...
  FIRST_LIVE_CONTAINER_NAME=$(cat /var/snap/lxd/common/lxd/logs/lxd.log | grep 'Started container' | grep -v '\-staging\-' | head -n1 | egrep -o "(instance|name)=.*" | cut -d'=' -f2 | cut -d' ' -f1)
  FIRST_LIVE_CONTAINER_TIME=$(cat /var/snap/lxd/common/lxd/logs/lxd.log | grep 'Started container' | grep "$FIRST_LIVE_CONTAINER_NAME" | head -n1 | cut -d' ' -f1 | egrep -o '[0-9]+:[0-9]+')
